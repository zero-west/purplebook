...
import me.zw.purplebook.config.auth.dto.OAuthAttributes;
import me.zw.purplebook.config.auth.dto.SessionUser;
import me.zw.purplebook.domain.user.User;
import me.zw.purplebook.domain.user.UserRepository;
...
public class CustomOauth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    private User saveOrUpdate(OAuthAttributes attributes) {
        User user = userRepository.findByEmail(attributes.getEmail())
                .map((var entity) -> entity.update(attributes.getName(), attributes.getPicture()))
